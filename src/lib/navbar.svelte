<script lang='ts'>
	import {LightSwitch } from '@skeletonlabs/skeleton';
</script>
<style>
.showMoreBtn:hover + ul,
.showMoreBtn:focus + ul{
	opacity: 1.0;
    transition: opacity .55s ease-in-out;
	visibility:visible;
}

.showMoreBtn:hover,.showMoreBtn:focus{
    outline: 4px solid  var(--dark-green);
}
ul{
	width: 100px;
	opacity: 0;
	visibility: hidden;
	transition: opacity 0.6s ease, visibility 0.6s ease;
}
ul:hover{
	opacity: 1;
	visibility:visible;
}
li{
	padding: 3px 0;
}
@media(max-width: 500px){
 ul{
	transform: translateX(-80px);
 }
}
a{
    text-decoration: none !important; 
    color: inherit !important;

}

</style>
<header class="bg-surface-100-800-token flex w-full justify-center items-center py-4 gap-2">
			<div class="mr-auto">
					<svg xmlns="http://www.w3.org/2000/svg" width="50" height="40" viewBox="0 0 256 308"><path fill="#FF3E00" d="M239.682 40.707C211.113-.182 154.69-12.301 113.895 13.69L42.247 59.356a82.198 82.198 0 0 0-37.135 55.056a86.566 86.566 0 0 0 8.536 55.576a82.425 82.425 0 0 0-12.296 30.719a87.596 87.596 0 0 0 14.964 66.244c28.574 40.893 84.997 53.007 125.787 27.016l71.648-45.664a82.182 82.182 0 0 0 37.135-55.057a86.601 86.601 0 0 0-8.53-55.577a82.409 82.409 0 0 0 12.29-30.718a87.573 87.573 0 0 0-14.963-66.244"/><path fill="#FFF" d="M106.889 270.841c-23.102 6.007-47.497-3.036-61.103-22.648a52.685 52.685 0 0 1-9.003-39.85a49.978 49.978 0 0 1 1.713-6.693l1.35-4.115l3.671 2.697a92.447 92.447 0 0 0 28.036 14.007l2.663.808l-.245 2.659a16.067 16.067 0 0 0 2.89 10.656a17.143 17.143 0 0 0 18.397 6.828a15.786 15.786 0 0 0 4.403-1.935l71.67-45.672a14.922 14.922 0 0 0 6.734-9.977a15.923 15.923 0 0 0-2.713-12.011a17.156 17.156 0 0 0-18.404-6.832a15.78 15.78 0 0 0-4.396 1.933l-27.35 17.434a52.298 52.298 0 0 1-14.553 6.391c-23.101 6.007-47.497-3.036-61.101-22.649a52.681 52.681 0 0 1-9.004-39.849a49.428 49.428 0 0 1 22.34-33.114l71.664-45.677a52.218 52.218 0 0 1 14.563-6.398c23.101-6.007 47.497 3.036 61.101 22.648a52.685 52.685 0 0 1 9.004 39.85a50.559 50.559 0 0 1-1.713 6.692l-1.35 4.116l-3.67-2.693a92.373 92.373 0 0 0-28.037-14.013l-2.664-.809l.246-2.658a16.099 16.099 0 0 0-2.89-10.656a17.143 17.143 0 0 0-18.398-6.828a15.786 15.786 0 0 0-4.402 1.935l-71.67 45.674a14.898 14.898 0 0 0-6.73 9.975a15.9 15.9 0 0 0 2.709 12.012a17.156 17.156 0 0 0 18.404 6.832a15.841 15.841 0 0 0 4.402-1.935l27.345-17.427a52.147 52.147 0 0 1 14.552-6.397c23.101-6.006 47.497 3.037 61.102 22.65a52.681 52.681 0 0 1 9.003 39.848a49.453 49.453 0 0 1-22.34 33.12l-71.664 45.673a52.218 52.218 0 0 1-14.563 6.398"/></svg>
			</div>
				<a
					class="btn btn-sm variant-ghost-surface"
					href="/"
					rel="noreferrer"
				>
				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M4 21V9l8-6l8 6v12h-6v-7h-4v7H4Z"/></svg>
				</a>
				<a
					class="btn btn-sm variant-ghost-surface"
					href="/kauppa"
					rel="noreferrer"
				>
				 Kauppa
				</a>
				<a
					class="btn btn-sm variant-ghost-surface"
					href="/galleria"
				>
				Galleria
			</a>
				<div class="relative">
					<button class="btn btn-sm variant-ghost-surface showMoreBtn">
						<svg xmlns="http://www.w3.org/2000/svg" width="22" height="20" viewBox="0 0 16 16"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.75 12.25h10.5m-10.5-4h10.5m-10.5-4h10.5"/></svg>
					</button>
				<ul class="absolute min-w-[19ch] text-center left-0 cursor-pointer translate-x-[-25px] p-2 rounded bg-surface-50-900-token">
					<a href="/info/ajanvaraus"><li class="hover:bg-surface-200-700-token">Ajanvaraus</li></a>
					<a href="/info/hinnasto"><li class="hover:bg-surface-200-700-token">Hinnasto</li></a>
					<a href="/info/susanna"><li class="hover:bg-surface-200-700-token">Susanna</li></a>
					<a href="/info/hyvinvointipalvelut"><li class="hover:bg-surface-200-700-token">Hyvinvointipalvelut</li></a>
					<a href="/info/tapahtumat"><li class="hover:bg-surface-200-700-token">Tapahtumat</li></a>
					<a href="/info/yhteystiedot"><li class="hover:bg-surface-200-700-token">Yhteystiedot</li></a>
					<a href="/info/tietosuoja"><li class="hover:bg-surface-200-700-token">Tietosuojaseloste</li></a>
				</ul>
			</div>
			<div class="ml-auto mr-2">
				<LightSwitch />
			</div>
	
</header>