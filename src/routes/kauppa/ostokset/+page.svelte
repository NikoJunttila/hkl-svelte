<script lang="ts">
	...
	const t: ToastSettings = {
		message:
			'Jokin meni pieleen. tarkasta annetut infot. Jos ei onnistu silti niin ota yhteyttä spostilla tai soita',
		...
		for (const element of data.data) {
			if (element.text === alennus) {
				...
				cartItems = cartItems.map((product) => {
					const discountedPrice = product.alennusHinta * alennusKerroin;
					return { ...product, hinta:discountedPrice };
				});
				return true;
			}
		}
		...
		<div class="flex flex-wrap items-center gap-3 py-4 justify-center flex-col">
			{#each cartItems as item, index}
				<div class="flex flex-col gap-2 items-center justify-center">
					...
					<span>{item.tuote} {item.hinta.toFixed(2)}€</span>
			...
			<form class="">
				...
				/>
				<p>lisätietoa:</p>
				<textarea class="input variant-form-material" bind:value={extratext} />
				<br />
				<p>Alennus koodi:</p>
				<input
					class="input variant-form-material"
				...
				<button
					...
					on:click={() => buyItems()}
					class="hover:dark:bg-[#0f0448] mb-10 hover:bg-blue-400 dark:bg-[var(--dark-green)] bg-[var(--light-green)] mt-3"
